...
void Guidance::Init() {
    ...
    auto xcorrpid = new PID(0.1, 0.1, -0.1, 0.1, 0.05, 0.01);

    xPids->push_back(xangvelpid);
    xPids->push_back(xangaccpid);
    xPids->push_back(xcorrpid);

    auto yrollangvelpid = new PID(0.1, 4, -4, 0.4, 0.05, 0.01);
    auto yrollangaccpid = new PID(0.1, 2, -2, 0.3, 0.05, 0.01);
    auto ycorrpid = new PID(0.1, 0.1, -0.1, 0.1, 0.05, 0.01);
...
//    //calculate PIDs
    double Xcorr = 0;
    double Ycorr = 0;
    Xcorr = mPIDpipelines.at(X)->Calculate(0, XaxisErrors)/5;
